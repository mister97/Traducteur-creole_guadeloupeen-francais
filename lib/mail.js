...
export async function mailSuggestionValidee(suggestion, slug) {
  ...
  const sujet = kr ? `${SITE_NOM} : mèsi pou popozisyon a-w !` : `${SITE_NOM} : merci pour votre proposition !`;
  const corps = kr
    ? `Bonjou${suggestion.nom ? ` ${suggestion.nom}` : ''},\n\nPopozisyon a-w${suggestion.mot ? ` pou « ${suggestion.mot} »` : ''} antré adan diksyonnè-la. Mèsi onlo pou koudmen-la !`
