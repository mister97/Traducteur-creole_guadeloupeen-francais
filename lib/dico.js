...
export async function lettresDisponibles() {
  const lignes = await requete(
    `SELECT UPPER(LEFT(recherche, 1)) AS lettre, COUNT(*) AS n
       FROM formes WHERE principale = 1 AND recherche <> ''
      GROUP BY UPPER(LEFT(recherche, 1)) ORDER BY lettre`,
  );
  return lignes.filter((l) => /^[A-Z]$/.test(l.lettre));
}

export async function motsParLettre(lettre, { limite = 60, decalage = 0 } = {}) {
  const l = lettre.toLowerCase();
  const [{ total }] = await requete(
    'SELECT COUNT(*) AS total FROM formes WHERE principale = 1 AND recherche LIKE ?',
    [`${l}%`],
  );
  const lignes = await requete(
    `SELECT e.id, e.mot, e.slug
       FROM formes f JOIN entrees e ON e.id = f.entree_id
      WHERE f.principale = 1 AND f.recherche LIKE ?
      ORDER BY e.mot, e.id
      LIMIT ? OFFSET ?`,
    [`${l}%`, limite, decalage],
  );
