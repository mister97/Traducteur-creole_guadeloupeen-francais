...
//   mot: 'MANJÉ',
//   variantes: ['MANGÉ'],
//   sens: [{ traduction: 'Nourrir, manger', synonymes: ['nannan'], termes: ['manger', 'nourrir'] }],
//   locutions: [{ expression: '', traduction: '', exemple_kr: '', exemple_fr: '' }],
// }

const LIMITES = { mot: 160, texte: 2000, liste: 60, sens: 60, locutions: 60, terme: 190 };
...
export function ficheVide() {
  return { mot: '', variantes: [], sens: [{ traduction: '', synonymes: [], termes: [] }], locutions: [] };
}
...
export function normaliserFiche(brut) {
  ...
  return {
    ...
    sens: (Array.isArray(b.sens) ? b.sens : [])
      ...
      .filter((s) => s.traduction || s.synonymes.length)
...
export function ficheEnLignes(fiche, { avecTermes = true } = {}) {
  ...
  fiche.locutions?.forEach((l) => {
    ...
    if (l.exemple_kr || l.exemple_fr) lignes.push(`   Exemple : ${l.exemple_kr}${l.exemple_fr ? ` → ${l.exemple_fr}` : ''}`);
  });
  return lignes;
}
