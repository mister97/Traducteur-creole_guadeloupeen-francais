import { tousLesSlugs } from '@/lib/dico';
...
export default async function sitemap() {
  const { mots, termes } = await tousLesSlugs();
  const fixes = ['', '/jeu', '/proposer', '/a-propos', '/mentions-legales', '/confidentialite', ...'abcdefghijklmnopqrstuvwxyz'.split('').map((l) => `/lettre/${l}`)];
  return [
